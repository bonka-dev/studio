...
#---------------------------------------------------------------------------------------------------------------------
TARGET      	:=  bonka_welcome
BUILD       	:=  build
LIBBUTANO   	:=  vendor/butano/butano
PYTHON      	:=  python3
SOURCES         :=  src vendor/butano/common/src
INCLUDES        :=  include vendor/butano/common/include
DATA        	:=
GRAPHICS        :=  graphics vendor/butano/common/graphics
AUDIO       	:=  audio
AUDIOBACKEND	:=  maxmod
AUDIOTOOL		:=
DMGAUDIO    	:=  dmg_audio
DMGAUDIOBACKEND	:=  default
ROMTITLE    	:=  BONKA START
ROMCODE     	:=  BNKA
USERFLAGS   	:=  -flto=auto
USERCXXFLAGS	:=
USERASFLAGS 	:=
...
STACKTRACE		:=
USERBUILD   	:=
EXTTOOL     	:=  node devtools/bonka/generate_assets.cjs --scene maps/world.json --asset world
